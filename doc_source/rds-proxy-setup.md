...
**Note**  
This security group must allow access from the proxy to the database\. The same security group is used for ingress from your applications to the proxy, and for egress from the proxy to the database\. For example, suppose that you use the same security group for your database and your proxy\. In this case, make sure that you specify that resources in that security group can communicate with other resources in the same security group\. Also if your proxy uses secrets manager your proxy security group outbound connection must allow connection to secrets manager.  
